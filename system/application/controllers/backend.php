...
class Backend extends Controller
{
	...
	function _save_ad()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			if($this->input->post('adress') != "") {
				...
				// Robust: DB-kolumnerna är NOT NULL -> skriv tom sträng om ingen träff
				$lat  = (is_array($geocode) && isset($geocode['latitude']) && $geocode['latitude'] !== null) ? (string)$geocode['latitude'] : "";
				$long = (is_array($geocode) && isset($geocode['longitude']) && $geocode['longitude'] !== null) ? (string)$geocode['longitude'] : "";
			} else {
				$lat = "";
				$long = "";
			}
			...
			if($this->input->post('field') != "") {
				foreach($this->input->post('field') as $row) {
					$reg_id = '';
					if((isset($row['main']) AND $row['main'] != "")) {
						$reg_id = $row['main'];
					}
	...
	function _update_ad($id)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			// ✅ FIX: category_update måste sättas, annars sparas "ingen kategori"
			$category_update = $this->input->post('category');
			if (is_array($category_update)) {
				$category_update = (count($category_update) > 0) ? $category_update[0] : '';
			}
			$category_update = trim((string)$category_update);
			...
			// Spara regionrelationer. Rensa först
